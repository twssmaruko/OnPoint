...
export const createMMR = () => async (dispatch, getState) => {
  ...
  const test = getState().materialsReceiving.materialsReceiving;
  console.log('materialsReceiving: ', test);
...
export const setCategories = (data) => async (dispatch) => {
  ...
  try {
    ...
    const foundProject = fetchedProjects.find((element) => element.projectCode === data)
    dispatch(fetchProjectsToStore(foundProject));

    const categories = [];

    for (const budgetCost in foundProject.budget.budgetCost) {
      for (const subCategories in foundProject.budget.budgetCost[budgetCost].subCategories) {
        for (const subCategoryItems in foundProject.budget
          .budgetCost[budgetCost]
          .subCategories[subCategories]
          .subCategoryItem) {

          const newCategories = foundProject.budget.budgetCost[budgetCost].itemCode
            + '.' + foundProject.budget.budgetCost[budgetCost].subCategories[subCategories].itemCode
            + '.' + foundProject.budget.budgetCost[budgetCost].subCategories[subCategories].subCategoryItem[subCategoryItems].itemCode
